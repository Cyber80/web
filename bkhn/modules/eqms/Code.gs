...
function doPost(e) {
  ...
  try {
    ...
    var readOnly = ['meta','data','exams/subject_overview','export/json','export/csv'].indexOf(req.action) >= 0;
...
function dispatch_(action,req) {
  if(action==='years/add') {
    ...
    var ss=SpreadsheetApp.create('EQMS • ปีการศึกษา '+y);
    Object.keys(EQMS_TABLES).forEach(function(n){writeSheetData_(ss,n,[]);});
