        ...
        function renderModalsData() {
            ...
            uniqueYears.forEach(y => {
                ...
                if (yearBody) {
                    const tr = document.createElement("tr");
                    tr.innerHTML = `
                        <td class="fw-bold">${y.year} ${y.is_active ? '<span class="badge bg-success ms-1">ใช้งานอยู่</span>' : ''}</td>
                        <td><code>${y.sheet_id || '-'}</code></td>
                        <td class="text-center">
                            <button class="btn btn-outline-danger btn-sm" onclick="deleteYear('${y.year}')" ${y.is_active ? 'disabled' : ''}><i class="fa-solid fa-trash"></i></button>
                        </td>
                    `;
            ...
            GLOBAL_DB.Config_Metadata.Subjects.forEach(s => {
                const tr = document.createElement("tr");
                const unitsList = (s.units || []).map(u => `<span class="badge bg-light text-dark border me-1">หน่วย ${u.unit_no}: ${u.unit_name}</span>`).join(" ");
                tr.innerHTML = `
        ...
        async function saveSubjectFromModal() {
            ...
            const res = await apiFetch("/api/subjects/save", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify(payload)
            });
            ...
            document.getElementById("editSubjGrade").value = "";
            try { bootstrap.Modal.getInstance(document.getElementById("subjectManagerModal")).hide(); } catch(e){}
        ...
        async function addNewYear() {
            ...
            const res = await apiFetch("/api/years/add", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ year: y })
            });
            ...
            await fetchData();
            alert("เพิ่มปีการศึกษาเรียบร้อย!");
        }
...
async function apiFetch(path,options={}) {
    ...
    const write=!['meta','data','exams/select','years/select','exams/subject_overview','export/json','export/csv'].includes(action);
